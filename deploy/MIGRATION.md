...
- [ ] Вернуть боевые значения через Telegram (порог/лимиты) — они сохранятся в `settings.json` на VPS и переживут рестарты/редеплои (файл не в git, `git pull` его не трогает)
